import { _ as ValidationError } from "./errors-B165yJUQ.mjs";
...
*/
const CreateCustomToolBaseSchema = z$1.object({
	name: z$1.string().min(1, "createCustomTool: name is required"),
	description: z$1.string().min(1, "createCustomTool: description is required"),
	/**
	...
	*/
	extendsToolkit: z$1.string().optional(),
	preload: z$1.boolean().optional().describe("When true, include this custom tool in session.tools() so it can be called without searching first.")
});
/**
* Zod schema for validating the string/scalar fields of createCustomToolkit() options.
* Slug is validated separately as the first argument.
*/
const CreateCustomToolkitBaseSchema = z$1.object({
	name: z$1.string().min(1, "createCustomToolkit: name is required"),
	description: z$1.string().min(1, "createCustomToolkit: description is required"),
	preload: z$1.boolean().optional().describe("When true, include tools from this custom toolkit in session.tools(). Tool-level preload values override this default.")
});
...
function validateSlugLength(toolSlug, toolkitSlug, context) {
	...
	if (finalLength > MAX_SLUG_LENGTH) {
		...
		throw new ValidationError(`${context}: slug "${toolSlug}" is too long. With prefix "${prefix}", the final slug would be ${finalLength} characters (max ${MAX_SLUG_LENGTH}). Shorten the slug to at most ${available} characters.`);
	}
}
/**
